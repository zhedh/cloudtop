...
const ApiTerritory = (props: Props) => {
  ...
  const getMapData = async () => {
    await getCnJson()
    const { result } = await performanceApiTerritory({
      startTime: startTime.format('YYYY-MM-DD HH:mm:ss'),
      endTime: endTime.format('YYYY-MM-DD HH:mm:ss'),
    })
    setTerritoryData(result)
    loadMap(result.map((i) => ({ value: +i.successRatio, name: i.key })))
  }
...
export default React.memo(ApiTerritory)
